...
// result of 1-wire transfer. Passing in callback parameter
typedef enum
{
	...
	OWMR_PENDING								// no result. Packed waiting for transfer in queue
} ow_result_t;

// Callback function of 1-wire packet. Invoks after transfer completion by 1-wire controlling module.
// Result of completed transfer and ptr to packet struct passes in callback parameters. 
// Higher level module, wich enque given packet, can instantly continue 1-wire activiti by modifying 
// packet and returning 1 from callback function. Packet will be retransmitted instead of
// the next packet from queue.
typedef uint32_t(*ow_packet_callback_t)(ow_result_t result, void* p_ow_packet);
  
// 1-wire transfer packet struct
typedef struct
{
